...
async fn test_runs_docker_and_echos() {
    ...
    assert_eq!(stream_string(output.stdout()).trim(), "hello");
    ...
    assert_eq!(stream_string(output.stdout()).trim(), "/usr/bin/rg");
    ...
    assert!(
        stream_string(output.stdout()).contains("src/tests.rs"),
    ...
    assert!(
        stream_string(output.stdout()).contains("Cargo.toml"),
        "{output:?} does not contain expected path"
    );
}
...
async fn test_shell_reads_etc_profile() {
    ...
    assert_eq!(stream_string(output.stdout()).trim(), "from_profile");
}
...
async fn test_context_present() {
    ...
    assert!(
        stream_string(ls.stdout()).contains("Cargo.toml"),
        "Context did not contain `Cargo.toml`, actual:\n {}",
        stream_string(ls.stdout())
    );
}
...
async fn test_current_dir_resolution() {
    ...
    assert_eq!(stream_string(default_pwd.stdout()).trim(), "/app");
    ...
    let relative_pwd = executor
        ...
        .await
        .unwrap();
    assert_eq!(stream_string(relative_pwd.stdout()).trim(), "/app/project");

    let absolute_pwd = executor
        .exec_cmd(&Command::shell("pwd").with_current_dir("/tmp"))
        .await
        .unwrap();
    assert_eq!(stream_string(absolute_pwd.stdout()).trim(), "/tmp");
    ...
    assert_eq!(stream_string(read_output.stdout()).trim(), "hello");
}
...
async fn test_default_workdir_override() {
    ...
    assert_eq!(stream_string(pwd.stdout()).trim(), "/tmp");
    ...
    let read_output = executor.exec_cmd(&read_cmd).await.unwrap();
    assert_eq!(stream_string(read_output.stdout()).trim(), "contents");
}
...
async fn test_overrides_include_git_respects_ignore() {
    ...
    assert!(stream_string(ls.stdout()).contains(".git"));
    assert!(!stream_string(ls.stdout()).contains("README.md"));
    assert!(!stream_string(ls.stdout()).contains("target"));
    assert!(!stream_string(ls.stdout()).contains("ignored_file"));
    ...
    assert!(stream_string(git_head.stdout()).contains("ref: refs/heads/"));
    ...
    eprintln!("{}", stream_string(git_status.stdout()));

    // It's ignored so git will think it's deleted
    assert!(stream_string(git_status.stdout()).contains("deleted:    ignored_file"));
}
...
async fn test_write_and_read_file_with_quotes() {
    ...
    assert_eq!(content, stream_string(read_file.stdout()));
}
...
async fn test_assert_container_stopped_on_drop() {
    ...
    let result = executor
        ...
        .await
        .unwrap();
    assert_eq!(stream_string(result.stdout()).trim(), "hello");
...
async fn test_run_multiline_bash_script() {
    ...
    let result = stream_string(output.stdout());
...
async fn test_clear_env() {
    ...
    let env_output = stream_string(output.stdout());
    dbg!(&env_output);
    assert!(
...
async fn test_default_timeout_triggers() {
    ...
    match err {
        CommandError::TimedOut { timeout, output } => {
            ...
            assert_eq!(stream_string(output.stdout()), "before-timeout");
        }
...
async fn test_timeout_kills_child_processes_and_executor_recovers() {
    ...
    let output = tokio::time::timeout(Duration::from_secs(3), executor.exec_cmd(&echo))
        ...
        .unwrap();
    assert_eq!(stream_string(output.stdout()).trim(), "healthy");

    let read_pid = Command::read_file(Path::new("/tmp/swiftide-timeout-child-pid"));
    let child_pid = stream_string(executor.exec_cmd(&read_pid).await.unwrap().stdout());
    let child_pid = child_pid.trim();

    let inspect = Command::shell(format!(
        r#"if [ -r /proc/{child_pid}/stat ]; then awk '{{ print $3 }}' /proc/{child_pid}/stat; else echo gone; fi"#
    ));
    let child_state = stream_string(executor.exec_cmd(&inspect).await.unwrap().stdout());
...
async fn test_timed_out_command_does_not_kill_concurrent_command() {
    ...
    assert_eq!(stream_string(output.stdout()).trim(), "survivor");
}
...
async fn test_background_shell_command_returns_immediately() {
    ...
    assert!(
        stream_string(output.stdout()).contains("Background command started")
            || stream_string(output.stdout()).trim().is_empty(),
    ...
    let echo = executor
        ...
        .await
        .unwrap();
    assert_eq!(stream_string(echo.stdout()).trim(), "done");
    ...
    assert_eq!(stream_string(output.stdout()), "ready\n");
}
